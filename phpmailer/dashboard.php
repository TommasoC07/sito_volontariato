...
if($_SERVER["REQUEST_METHOD"]==="POST" && isset($_POST['donation'])){
    ...
    echo "<div class='alert alert-success alert-dismissible fade show'> Donazione effettuata con successo <button type='button' class='btn-close' data-bs-dismiss='alert'></button> </div>";
}
...
<div class="bg-light border rounded-pill px-4 py-2 shadow-sm" style="width: fit-content; margin-top: 20px;">
        <span class="text-muted fw-semibold me-2">TOTALE DONATO</span>
<span class="fs-3 fw-bold text-success">
            <?php  
                $sql = "SELECT SUM(importo) AS totale FROM donazione WHERE id_utente = :id_utente";
                $stmt = $pdo->prepare($sql);
                $stmt->execute(['id_utente' => $_SESSION['user_id']]);
